...
from utils.utils import union_all
...
for key, value in auxilio_paths.items():
    path = f"{value}/"
    if full_load:
        df_auxilio = spark.read.table(f"data_lakehouse.default.silver_{key}")
    else: 
        df_auxilio = spark.read.table(f"data_lakehouse.default.silver_{key}").where(partition_condition)
